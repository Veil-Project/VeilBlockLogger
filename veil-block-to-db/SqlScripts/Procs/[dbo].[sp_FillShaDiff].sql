...
CREATE PROCEDURE [dbo].[sp_FillShaDiff] 
AS
BEGIN

	DECLARE @StartBlock bigint = (SELECT ISNULL(MAX(BLOCKID),0) + 1 FROM bak.ShaDiffHistory);
	DECLARE @BlockType int = 31; 
	SELECT BDO.BlockID, 
	...
	INSERT INTO bak.ShaDiffHistory(BlockID,PowDiff,Ma720Blocks,Ma1440Blocks, Ma4320Blocks, Ma10080Blocks)	
		SELECT BDO.BlockID,
		BDO.PowDiff,	
